...
class ZYPageControl: UIView {
    ...
    // pageControl的样式
    fileprivate var style : ZYPageControlStyle = .original
    
    // 初始化方法
    convenience init(frame: CGRect, style : ZYPageControlStyle) {
    ...
    fileprivate func setPage() {
        ...
        case .ring:
            ...
            break
        case .number:
            let label = self.viewWithTag(curLabelTag) as! UILabel
    ...
    override func layoutSubviews() {
        ...
        if self.subviews.count == 0 {       // 避免重复创建子控件
            layoutPages()
        }
    }
}
